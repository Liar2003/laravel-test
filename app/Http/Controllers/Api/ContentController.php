...
class ContentController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $contents = Content::query()
            ->when($search, function ($query, $search) {
                // Use a grouped where clause for correct logic
                return $query->where(function ($q) use ($search) {
                    $searchTerm = '%' . strtolower($search) . '%';

                    // Apply the case-insensitive search
                    $q->whereRaw('LOWER(title) LIKE ?', [$searchTerm])
                        ->orWhereRaw('LOWER(content) LIKE ?', [$searchTerm]);
                });
            })
            ->select('id', 'title', 'profileImg', 'coverImg', 'category', 'content', 'tags', 'isvip', 'created_at')
    ...
    public function getHomeContents(Request $request)
    {
        ...
        $specialContent = [
            'vip_contents' => Content::where('category', 'Jav')
                ...
                ->take(8)
                ->get(),
            'liveAndsport' => Content::whereIn('category', ['Football', 'MMA'])
                ->select(['id', 'title', 'profileImg', 'coverImg', 'tags', 'content', 'category', 'duration', 'isvip', 'created_at'])
                ->take(8)
                ->get()
        ];
        ...
        $additionalData = [
            ...
            'announce' => Announce::whereIn('category', ['announce'])->get()
        ];

        // Step 5: Handle ads
        $ad = [];
        // $announce = ['announce' => [['link' => '🌻...မင်္ဂလာရှိသောနေ့ရက်လေး ဖြစ်ကြပါစေ...🌻', 'imgUrl' => ''],['link' => 'VIP စျေးနှုန်းများ 1 လ = 700 ကျပ် 3 လ = 1500 ကျပ် 5 လ = 3000 ကျပ်', 'imgUrl' => '']]];
        if (!$request->device->isVip()) {
            $ad = ["ad" => Announce::whereIn('category', ['ads'])->get()];
    ...
    public function getContentsByCategory(Request $request, $category)
    {
        ...
        $query = Content::where('category', $category)
            ->select('id', 'title', 'profileImg', 'coverImg', 'content', 'category', 'tags', 'isvip', 'created_at')->orderBy('created_at', 'desc');
    ...
    public function getContentsByCast(Request $request, $cast)
    {
        ...
        $query = Content::whereRaw("EXISTS (
            ...
            ->select('id', 'title', 'profileImg', 'coverImg', 'content', 'category', 'tags', 'isvip', 'created_at')
            ->orderBy('created_at', 'desc');
    ...
    public function getContentsByTag(Request $request, $tag)
    {
        ...
        $contents = Content::whereJsonContains('tags', $tag)
            ->select('id', 'title', 'profileImg', 'coverImg', 'tags', 'content', 'category', 'isvip', 'created_at')
            ->paginate($perPage, ['*'], 'page', $page);
    ...
    public function listContents(Request $request)
    {
        ...
        $query = Content::select('id', 'title', 'profileImg', 'coverImg', 'tags', 'content', 'category', 'duration', 'isvip', 'created_at')
            ->whereNotIn('category', ['Live', 'Sport', 'Football', 'MMA', 'Myanmar']) // Exclude "Live" and "Sport"
            ->orderBy('created_at', 'desc');
    ...
    public function listLiveAndSportContents(Request $request)
    {
        ...
        $query = Content::select('id', 'title', 'profileImg', 'coverImg', 'tags', 'content', 'category', 'duration', 'isvip', 'created_at')
            ->whereIn('category', ['Live', 'Sport', 'MMA']) // Only include "Live" and "Sport"
            ->orderBy('created_at', 'desc');
