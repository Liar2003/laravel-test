...

{
    public function listContents(Request $request)
    {
        ...
        $query = Content::select('id', 'title', 'profileImg', 'coverImg', 'tags', 'content', 'category', 'duration', 'isvip', 'created_at')
            ->whereNotIn('category', ['Live', 'Sport','MMA']) // Exclude "Live" and "Sport"
    ...
    public function listDevices(Request $request)
    {
        ...
        return response()->json([
            'devices' => collect($devices->items())->map(function ($device) {
                $array = $device->toArray();
                $array['referral_count'] = $device->device_referrals_count;
                return $array;
            }),
            'pagination' => [
                'total' => $devices->total(),
    ...
    public function createSubscriptionKey(Request $request)
    {
        $request->validate([
            ...
            'max_devices' => 'required|integer|min:1|max:100' // Max 5 devices per key
        ]);
